...
ex_path = Path(f'./experiments/simple_transport_cnn_single.yml')
...
for d in train_dl:
    print(d.shape)
    frames = [plot(d[i,0,:].reshape(-1,1) ) for i in range(d.shape[0])]

    gif.save(frames, f'simple_transport_cnn.gif', duration=50)
